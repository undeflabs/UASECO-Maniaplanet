...
class PluginRecordsEyepiece extends Plugin {
	...
	*/
	...
	public function __construct () {
		...
		$this->addDependence('PluginAlternateScoretable',	Dependence::WANTED,	'1.0.0', null);
	...
	public function onSync ($aseco, $reload = null) {
		...
		if ( defined('UASECO_VERSION') ) {
			...
			if ( version_compare($version, $uaseco_min_version, '<') ) {
				trigger_error('[RecordsEyepiece] Not supported USAECO version ('. $version .')! Please update to min. version '. $uaseco_min_version .'!', E_USER_ERROR);
			}
		}
		else {
...
//				Gameinfo::TEAMATTACK	=> array('name' => 'TEAM ATTACK',	'icon' => 'RT_Team'),
	...
	*/
	...
	public function onShutdown ($aseco) {
		...
		$aseco->client->queryIgnoreResult('ManualFlowControlEnable', false);
	...
	*/
	...
	public function chat_estat ($aseco, $command) {
	...
	public function chat_elist ($aseco, $login, $chat_command, $chat_parameter) {
		...
		if ($aseco->server->gamestate == Server::RACE) {
			...
			if (count($player->data['RecordsEyepiece']['Maplist']['Records']) == 0) {
				...
				$player->data['RecordsEyepiece']['Maplist']['Records'] = $this->getPlayerLocalRecords($player->id);

				// Load all Maps that the calling Player did not finished yet
				$player->data['RecordsEyepiece']['Maplist']['Unfinished'] = $this->getPlayerUnfinishedMaps($player->id);
			}
			...
			if (strtoupper($chat_parameter) == 'JUKEBOX') {
				...
				if (strlen($chat_parameter) > 0) {
					...
					$player->data['RecordsEyepiece']['Maplist']['Filter'] = array('key' => $chat_parameter);
	...
	*/
	...
	public function onEverySecond ($aseco) {
		...
		if (time() >= $this->config['States']['RefreshTimestampPreload']) {
			...
			foreach ($aseco->server->players->player_list as $player) {
				if ( (time() >= $player->data['RecordsEyepiece']['Preload']['Timestamp']) && ($player->data['RecordsEyepiece']['Preload']['LoadedPart'] != 5) ) {
					$player->data['RecordsEyepiece']['Preload']['LoadedPart'] += 1;
					$widgets = $this->buildImagePreload($player->data['RecordsEyepiece']['Preload']['LoadedPart']);
	...
	*/
	...
	public function onPlayerConnect ($aseco, $player) {
		...
		$player->data['RecordsEyepiece']['Window']['Action'] = false;
		$player->data['RecordsEyepiece']['Window']['Page'] = 0;
		$player->data['RecordsEyepiece']['Window']['MaxPage'] = 0;
		$player->data['RecordsEyepiece']['Maplist']['Filter'] = false;
		$player->data['RecordsEyepiece']['Maplist']['Records'] = array();
		$player->data['RecordsEyepiece']['Maplist']['CurrentDisplay'] = array();
	...
	*/
	...
	public function onPlayerManialinkPageAnswer ($aseco, $answer) {
		...
		if ($answer[2] == 382009003) {
			...
			$player->data['RecordsEyepiece']['Window']['Action'] = 'showMaplistWindow';
			$player->data['RecordsEyepiece']['Window']['Page'] = 0;
			$player->data['RecordsEyepiece']['Maplist']['Filter'] = array('environment' => 'CANYON');
			$require_action = true;
			...
			$player->data['RecordsEyepiece']['Window']['Action'] = 'showMaplistWindow';
			$player->data['RecordsEyepiece']['Window']['Page'] = 0;
			$player->data['RecordsEyepiece']['Maplist']['Filter'] = array('environment' => 'STADIUM');
			$require_action = true;
			...
			$player->data['RecordsEyepiece']['Window']['Action'] = 'showMaplistWindow';
			$player->data['RecordsEyepiece']['Window']['Page'] = 0;
			$player->data['RecordsEyepiece']['Maplist']['Filter'] = array('environment' => 'VALLEY');
			$require_action = true;
			...
			$player->data['RecordsEyepiece']['Window']['Action'] = 'showMaplistWindow';
	//		$player->data['RecordsEyepiece']['Window']['Page'] = 0;						// already setup in chat_elist()!
	//		$player->data['RecordsEyepiece']['Maplist']['Filter'] = array('key' => $command['params']);	// already setup in chat_elist()!
			$require_action = true;
			...
			$item = array();
			$item['name']		= $player->data['RecordsEyepiece']['Maplist']['CurrentDisplay'][$id]['name'];
			$item['author']		= $player->data['RecordsEyepiece']['Maplist']['CurrentDisplay'][$id]['author'];
			$item['environment']	= $player->data['RecordsEyepiece']['Maplist']['CurrentDisplay'][$id]['environment'];
			$item['filename']	= $player->data['RecordsEyepiece']['Maplist']['CurrentDisplay'][$id]['filename'];
			$item['uid']		= $player->data['RecordsEyepiece']['Maplist']['CurrentDisplay'][$id]['uid'];
			...
			$current_page = ((($player->data['RecordsEyepiece']['Window']['Page'] + 1) * 80) - 80);
			...
			$player->data['RecordsEyepiece']['Window']['Action'] = 'showMaplistWindow';
			$player->data['RecordsEyepiece']['Window']['Page'] = 0;
			$player->data['RecordsEyepiece']['Maplist']['Filter'] = array('author' => $this->cache['MapAuthors'][$id]);
			$require_action = true;
			...
			if ($this->config['FEATURES'][0]['SONGLIST'][0]['FORCE_SONGLIST'][0] == true) {
				...
				$player->data['RecordsEyepiece']['Window']['Action'] = 'MusiclistWindow';
				$require_action = true;
			}
			...
			if ($player->data['RecordsEyepiece']['Window']['Action'] != $answer[2]) {
				$player->data['RecordsEyepiece']['Window']['Page'] = 0;
			}
			$player->data['RecordsEyepiece']['Window']['Action'] = $answer[2];
			$require_action = true;

		}
		else if ($answer[2] == 'WindowPagePrev') {

			$player->data['RecordsEyepiece']['Window']['Page'] -= 1;
			...
			$player->data['RecordsEyepiece']['Window']['Page'] -= 2;
			if ($player->data['RecordsEyepiece']['Window']['Page'] < 0) {
				$player->data['RecordsEyepiece']['Window']['Page'] = 0;
			}
			$require_action = true;
			...
			$player->data['RecordsEyepiece']['Window']['Page'] += 1;
			if ($player->data['RecordsEyepiece']['Window']['Page'] > $player->data['RecordsEyepiece']['Window']['MaxPage']) {
				$player->data['RecordsEyepiece']['Window']['Page'] = $player->data['RecordsEyepiece']['Window']['MaxPage'];
			}
			$require_action = true;
		...
		if ($require_action == true) {
			if ($player->data['RecordsEyepiece']['Window']['Action'] == 'showDedimaniaRecordsWindow') {

				$widgets .= $this->buildDedimaniaRecordsWindow($player->data['RecordsEyepiece']['Window']['Page']);

			}
			else if ($player->data['RecordsEyepiece']['Window']['Action'] == 'showLocalRecordsWindow') {

				$result = $this->buildLocalRecordsWindow($player->data['RecordsEyepiece']['Window']['Page']);
				$player->data['RecordsEyepiece']['Window']['MaxPage'] = $result['maxpage'];
				$widgets .= $result['xml'];

			}
			else if ($player->data['RecordsEyepiece']['Window']['Action'] == 'showLiveRankingsWindow') {
				...
				if ( function_exists('ast_showScoretable') ) {
					...
					$result = $this->buildLiveRankingsWindow($player->data['RecordsEyepiece']['Window']['Page']);
					$player->data['RecordsEyepiece']['Window']['MaxPage'] = $result['maxpage'];
					$widgets .= $result['xml'];
				}

			}
			else if ( ($player->data['RecordsEyepiece']['Window']['Action'] == 'showMusiclistWindow') || ($player->data['RecordsEyepiece']['Window']['Action'] == 'dropCurrentJukedSong') ) {

				$result = $this->buildMusiclistWindow($player->data['RecordsEyepiece']['Window']['Page'], $player->login);
				$player->data['RecordsEyepiece']['Window']['MaxPage'] = $result['maxpage'];
				$widgets .= $result['xml'];

			}
			else if ($player->data['RecordsEyepiece']['Window']['Action'] == 'showMaplistWindow') {

				$result = $this->buildMaplistWindow($player->data['RecordsEyepiece']['Window']['Page'], $player);
				$player->data['RecordsEyepiece']['Window']['MaxPage'] = $result['maxpage'];
				$widgets .= $result['xml'];

			}
			else if ($player->data['RecordsEyepiece']['Window']['Action'] == 'showMapAuthorlistWindow') {

				$result = $this->buildMapAuthorlistWindow($player->data['RecordsEyepiece']['Window']['Page'], $player);
				$player->data['RecordsEyepiece']['Window']['MaxPage'] = $result['maxpage'];
				$widgets .= $result['xml'];

			}
			else if ($player->data['RecordsEyepiece']['Window']['Action'] == 'showHelpWindow') {

				$player->data['RecordsEyepiece']['Window']['MaxPage'] = 2;
				$widgets .= $this->buildHelpWindow($player->data['RecordsEyepiece']['Window']['Page']);

			}
			else if ($player->data['RecordsEyepiece']['Window']['Action'] == 'showToplistWindow') {

				$result = $this->buildToplistWindow($player->data['RecordsEyepiece']['Window']['Page']);
				$player->data['RecordsEyepiece']['Window']['MaxPage'] = $result['maxpage'];
				$widgets .= $result['xml'];

			}
			else if ($player->data['RecordsEyepiece']['Window']['Action'] == 'showTopRankingsWindow') {

				$result = $this->buildScorelistWindowEntry($player->data['RecordsEyepiece']['Window']['Page'], 'TOP_RANKINGS', array('score', 'nickname'));
				$player->data['RecordsEyepiece']['Window']['MaxPage'] = $result['maxpage'];
				$widgets .= $result['xml'];

			}
			else if ($player->data['RecordsEyepiece']['Window']['Action'] == 'showTopWinnersWindow') {

				$result = $this->buildScorelistWindowEntry($player->data['RecordsEyepiece']['Window']['Page'], 'TOP_WINNERS', array('score', 'nickname'));
				$player->data['RecordsEyepiece']['Window']['MaxPage'] = $result['maxpage'];
				$widgets .= $result['xml'];

			}
			else if ($player->data['RecordsEyepiece']['Window']['Action'] == 'showMostRecordsWindow') {

				$result = $this->buildScorelistWindowEntry($player->data['RecordsEyepiece']['Window']['Page'], 'MOST_RECORDS', array('score', 'nickname'));
				$player->data['RecordsEyepiece']['Window']['MaxPage'] = $result['maxpage'];
				$widgets .= $result['xml'];

			}
			else if ($player->data['RecordsEyepiece']['Window']['Action'] == 'showMostFinishedWindow') {

				$result = $this->buildScorelistWindowEntry($player->data['RecordsEyepiece']['Window']['Page'], 'MOST_FINISHED', array('score', 'nickname'));
				$player->data['RecordsEyepiece']['Window']['MaxPage'] = $result['maxpage'];
				$widgets .= $result['xml'];

			}
			else if ($player->data['RecordsEyepiece']['Window']['Action'] == 'showTopPlaytimeWindow') {

				$result = $this->buildScorelistWindowEntry($player->data['RecordsEyepiece']['Window']['Page'], 'TOP_PLAYTIME', array('score', 'nickname'));
				$player->data['RecordsEyepiece']['Window']['MaxPage'] = $result['maxpage'];
				$widgets .= $result['xml'];

			}
			else if ($player->data['RecordsEyepiece']['Window']['Action'] == 'showTopDonatorsWindow') {

				$result = $this->buildScorelistWindowEntry($player->data['RecordsEyepiece']['Window']['Page'], 'TOP_DONATORS', array('score', 'nickname'));
				$player->data['RecordsEyepiece']['Window']['MaxPage'] = $result['maxpage'];
				$widgets .= $result['xml'];

			}
			else if ($player->data['RecordsEyepiece']['Window']['Action'] == 'showTopMapsWindow') {

				$result = $this->buildScorelistWindowEntry($player->data['RecordsEyepiece']['Window']['Page'], 'TOP_MAPS', array('karma', 'map'));
				$player->data['RecordsEyepiece']['Window']['MaxPage'] = $result['maxpage'];
				$widgets .= $result['xml'];

			}
			else if ($player->data['RecordsEyepiece']['Window']['Action'] == 'showTopVotersWindow') {

				$result = $this->buildScorelistWindowEntry($player->data['RecordsEyepiece']['Window']['Page'], 'TOP_VOTERS', array('score', 'nickname'));
				$player->data['RecordsEyepiece']['Window']['MaxPage'] = $result['maxpage'];
				$widgets .= $result['xml'];

			}
			else if ($player->data['RecordsEyepiece']['Window']['Action'] == 'showTopActivePlayersWindow') {

				$result = $this->buildScorelistWindowEntry($player->data['RecordsEyepiece']['Window']['Page'], 'TOP_ACTIVE_PLAYERS', array('score', 'nickname'));
				$player->data['RecordsEyepiece']['Window']['MaxPage'] = $result['maxpage'];
				$widgets .= $result['xml'];

			}
			else if ($player->data['RecordsEyepiece']['Window']['Action'] == 'showTopWinningPayoutWindow') {

				$result = $this->buildScorelistWindowEntry($player->data['RecordsEyepiece']['Window']['Page'], 'TOP_WINNING_PAYOUTS', array('won', 'nickname'));
				$player->data['RecordsEyepiece']['Window']['MaxPage'] = $result['maxpage'];
				$widgets .= $result['xml'];

			}
			else if ($player->data['RecordsEyepiece']['Window']['Action'] == 'showTopBetwinsWindow') {

				$result = $this->buildScorelistWindowEntry($player->data['RecordsEyepiece']['Window']['Page'], 'TOP_BETWINS', array('won', 'nickname'));
				$player->data['RecordsEyepiece']['Window']['MaxPage'] = $result['maxpage'];
				$widgets .= $result['xml'];

			}
			else if ($player->data['RecordsEyepiece']['Window']['Action'] == 'showTopRoundscoreWindow') {

				$result = $this->buildScorelistWindowEntry($player->data['RecordsEyepiece']['Window']['Page'], 'TOP_ROUNDSCORE', array('score', 'nickname'));
				$player->data['RecordsEyepiece']['Window']['MaxPage'] = $result['maxpage'];
				$widgets .= $result['xml'];

			}
			else if ($player->data['RecordsEyepiece']['Window']['Action'] == 'showTopVisitorsWindow') {

				$result = $this->buildScorelistWindowEntry($player->data['RecordsEyepiece']['Window']['Page'], 'TOP_VISITORS', array('score', 'nickname'));
				$player->data['RecordsEyepiece']['Window']['MaxPage'] = $result['maxpage'];
				$widgets .= $result['xml'];

			}
			else if ($player->data['RecordsEyepiece']['Window']['Action'] == 'showTopNationsWindow') {

				$result = $this->buildTopNationsWindow($player->data['RecordsEyepiece']['Window']['Page']);
				$player->data['RecordsEyepiece']['Window']['MaxPage'] = $result['maxpage'];
				$widgets .= $result['xml'];

			}
			else if ($player->data['RecordsEyepiece']['Window']['Action'] == 'showTopContinentsWindow') {

				$widgets .= $this->buildTopContinentsWindow();

			}
			else if ($player->data['RecordsEyepiece']['Window']['Action'] == 'showMaplistFilterWindow') {

				$widgets .= $this->buildMaplistFilterWindow();

			}
			else if ($player->data['RecordsEyepiece']['Window']['Action'] == 'showMaplistSortingWindow') {
...
//				$this->config['RoundScore']['Points'][Gameinfo::TEAM] = $aseco->server->gameinfo->team['PointsRepartition'];
	...
	*/
	...
	public function onBeginMap1 ($aseco, $map_item) {
		...
		$aseco->client->queryIgnoreResult('ManualFlowControlEnable', true);
		...
		if ($this->config['MUSIC_WIDGET'][0]['ENABLED'][0] == true) {
			...
			if ($this->config['States']['NiceMode'] == false) {
				foreach ($aseco->server->players->player_list as $player) {
					...
					if ( ($player->data['RecordsEyepiece']['Prefs']['WidgetState'] == true) && ($this->cache['MusicWidget'] != false) ) {
		...
		$this->config['PlacementPlaceholders']['MAP_NAME']			= $this->cache['Map']['Current']['name'];
		...
		if ($this->config['States']['NiceMode'] == false) {
			...
			foreach ($player_list as $player) {
				...
				if ( (($buildLiveRankingsWidget == true) || ($force['LiveRankings'] == true)) && ($this->config['LIVE_RANKINGS'][0]['GAMEMODE'][0][$gamemode][0]['ENABLED'][0] == true) ) {
					$widgets .= $this->buildLiveRankingsWidget($player->login, $player->data['RecordsEyepiece']['Prefs']['WidgetEmptyEntry'], $gamemode, $this->config['LIVE_RANKINGS'][0]['GAMEMODE'][0][$gamemode][0]['ENTRIES'][0]);
				}
...
//				'LiveRankingsWidget',
	...
	*/
	...
	public function buildWinningPayoutWidget () {
		...
		if ( ($server_planets - $players_planets) > $this->config['WINNING_PAYOUT'][0]['MINIMUM_SERVER_PLANETS'][0]) {
	...
	*/
	...
	public function winningPayout ($player) {
		...
		if ($this->cache['PlayerWinnings'][$player->login]['FinishPayment'] > 0) {
			...
			$aseco->client->query('Pay', (string)$player->login, (int)$this->cache['PlayerWinnings'][$player->login]['FinishPayment'], (string)$aseco->formatColors($message) );
			$billid = $aseco->client->getResponse();

			// Is there an error on pay?
			if ( $aseco->client->isError() ) {
				$aseco->console('[RecordsEyepiece] Pay '. $this->cache['PlayerWinnings'][$player->login]['FinishPayment'] .' Planets to Player "'. $player->login .'" failed: [' . $aseco->client->getErrorCode() . '] ' . $aseco->client->getErrorMessage());
			}
			else {
			...
			foreach ($aseco->server->rankings->ranking_list as $login => $data) {
				if ( ($data['time'] > 0) || ($data['score'] > 0) ) {
					...
					if ($gamemode == Gameinfo::ROUNDS) {
						...
						$this->scores['LiveRankings'][$i]['rank']	= $data['rank'];	// Fake 'Rank' set in $this->getCurrentRanking()
						$this->scores['LiveRankings'][$i]['score']	= $data['score'];
					}
	...
	*/
	...
	public function buildRecordWidgetContent ($gamemode, $player, $limit = 100, $widget) {
		...
		$preset = false;
		if ($player !== false) {
			$preset = $player->data['RecordsEyepiece']['Prefs']['WidgetEmptyEntry'];
		}
	...
	*/
	...
	public function buildDedimaniaRecordsWidgetBody ($gamemode) {
		...
		if ($topcount > 0) {
			$build['header'] .= '<quad posn="0.4 -2.6 0.004" sizen="'. ($this->config['DEDIMANIA_RECORDS'][0]['WIDTH'][0] - 0.8) .' '. (($topcount * $this->config['LineHeight']) + 0.3) .'" style="'. $this->config['STYLE'][0]['WIDGET_RACE'][0]['TOP_STYLE'][0] .'" substyle="'. $this->config['STYLE'][0]['WIDGET_RACE'][0]['TOP_SUBSTYLE'][0] .'"/>';
		}
...
EOL;
	...
	*/
	...
	public function buildLocalRecordsWidgetBody ($gamemode) {
		...
		// Add Background for top X Players
		if ($topcount > 0) {
			$build['header'] .= '<quad posn="0.4 -2.6 0.004" sizen="'. ($this->config['LOCAL_RECORDS'][0]['WIDTH'][0] - 0.8) .' '. (($topcount * $this->config['LineHeight']) + 0.3) .'" style="'. $this->config['STYLE'][0]['WIDGET_RACE'][0]['TOP_STYLE'][0] .'" substyle="'. $this->config['STYLE'][0]['WIDGET_RACE'][0]['TOP_SUBSTYLE'][0] .'"/>';
		}
...
EOL;
	...
	*/
	...
	public function buildLiveRankingsWidgetMS ($gamemode, $limit = 50) {
		...
		// Add Background for top X Players
		if ($topcount > 0) {
			$xml .= '<quad posn="0.4 -2.6 0.004" sizen="'. ($this->config['LIVE_RANKINGS'][0]['WIDTH'][0] - 0.8) .' '. (($topcount * $this->config['LineHeight']) + 0.2) .'" style="'. $this->config['STYLE'][0]['WIDGET_RACE'][0]['TOP_STYLE'][0] .'" substyle="'. $this->config['STYLE'][0]['WIDGET_RACE'][0]['TOP_SUBSTYLE'][0] .'"/>';
		}
		...
		$xml .= '<quad posn="0.4 -0.3 0.004" sizen="'. ($this->config['LIVE_RANKINGS'][0]['WIDTH'][0] - 0.8) .' 2" style="'. $this->config['STYLE'][0]['WIDGET_RACE'][0]['HIGHLITE_SELF_STYLE'][0] .'" substyle="'. $this->config['STYLE'][0]['WIDGET_RACE'][0]['HIGHLITE_SELF_SUBSTYLE'][0] .'" modulatecolor="05F" id="RecordsEyepieceLiveRankingsBackgroundMarker" hidden="true"/>';
...
EOL;
	...
	*/
	...
	public function buildLiveRankingsWidgetBody ($gamemode) {
		...
		// Add Background for top X Players
		if ($topcount > 0) {
			$build['header'] .= '<quad posn="0.4 -2.6 0.004" sizen="'. ($this->config['LIVE_RANKINGS'][0]['WIDTH'][0] - 0.8) .' '. (($topcount * $this->config['LineHeight']) + 0.3) .'" style="'. $this->config['STYLE'][0]['WIDGET_RACE'][0]['TOP_STYLE'][0] .'" substyle="'. $this->config['STYLE'][0]['WIDGET_RACE'][0]['TOP_SUBSTYLE'][0] .'"/>';
		}
...
//	$aseco->dump('RoundScore', $this->config['RoundScore']['Points'], $round_score);
			...
			foreach ($round_score as $item) {
				...
				if ($position == 'left') {
					if ($gamemode == Gameinfo::TEAM) {
						...
						$xml .= '<quad posn="-7.1 -'. ($this->config['LineHeight'] * $line + $offset - 0.3) .' 0.003" sizen="7 2" style="BgsPlayerCard" substyle="ProgressBar"/>';
						$xml .= '<label posn="-2.4 -'. ($this->config['LineHeight'] * $line + $offset) .' 0.004" sizen="4.8 2" halign="right" scale="0.9" textcolor="'. (($item['checkpointid'] < $round_score[0]['checkpointid']) ? 'D02F' : '0B3F').'" text="$O+'. $this->config['STYLE'][0]['WIDGET_RACE'][0]['FORMATTING_CODES'][0] . $aseco->formatTime(abs($item['scoplain'] - $round_score[0]['scoplain'])) .'"/>';
						$xml .= '<label posn="-0.4 -'. ($this->config['LineHeight'] * $line + $offset) .' 0.004" sizen="1.3 2" halign="right" scale="0.9" textcolor="'. (($item['checkpointid'] < $round_score[0]['checkpointid']) ? 'D02F' : '0B3F').'" text="$O'. $this->config['STYLE'][0]['WIDGET_RACE'][0]['FORMATTING_CODES'][0] . ($item['checkpointid']+1) .'"/>';
					}
					else {
						// Gameinfo::ROUNDS or Gameinfo::CUP
						$xml .= '<quad posn="-4.1 -'. ($this->config['LineHeight'] * $line + $offset - 0.3) .' 0.003" sizen="4 2" style="BgsPlayerCard" substyle="ProgressBar"/>';
						$xml .= '<label posn="-0.6 -'. ($this->config['LineHeight'] * $line + $offset) .' 0.004" sizen="3 2" halign="right" scale="0.9" textcolor="0B3F" text="$O+'. $this->config['STYLE'][0]['WIDGET_RACE'][0]['FORMATTING_CODES'][0] . $points .'"/>';
					}
				}
				else {
					if ($gamemode == Gameinfo::TEAM) {
						...
						$xml .= '<label posn="'. ($this->config['ROUND_SCORE'][0]['WIDTH'][0] + 4.6) .' -'. ($this->config['LineHeight'] * $line + $offset) .' 0.004" sizen="4.8 2" halign="right" scale="0.9" textcolor="'. (($item['checkpointid'] < $round_score[0]['checkpointid']) ? 'D02F' : '0B3F').'" text="$O+'. $this->config['STYLE'][0]['WIDGET_RACE'][0]['FORMATTING_CODES'][0] . $aseco->formatTime(abs($item['scoplain'] - $round_score[0]['scoplain'])) .'"/>';
						$xml .= '<label posn="'. ($this->config['ROUND_SCORE'][0]['WIDTH'][0] + 7) .' -'. ($this->config['LineHeight'] * $line + $offset) .' 0.004" sizen="1.3 2" halign="right" scale="0.9" textcolor="'. (($item['checkpointid'] < $round_score[0]['checkpointid']) ? 'D02F' : '0B3F').'" text="$O'. $this->config['STYLE'][0]['WIDGET_RACE'][0]['FORMATTING_CODES'][0] . ($item['checkpointid']+1) .'"/>';
					}
					else {
						// Gameinfo::ROUNDS or Gameinfo::CUP
						...
						$xml .= '<label posn="'. ($this->config['ROUND_SCORE'][0]['WIDTH'][0] + 3.6) .' -'. ($this->config['LineHeight'] * $line + $offset) .' 0.004" sizen="3 2" halign="right" scale="0.9" textcolor="0B3F" text="$O+'. $this->config['STYLE'][0]['WIDGET_RACE'][0]['FORMATTING_CODES'][0] . $points .'"/>';
					}
				}
		...
		if ($this->config['States']['NiceMode'] == false) {
			if ($item['self'] == -1) {
				...
				if ( (isset($item['highlitefull'])) && ($item['highlitefull']) ) {
					...
					$xml .= '<quad posn="0.4 -'. ($this->config['LineHeight'] * $line + $offset - 0.3) .' 0.004" sizen="'. ($widgetwidth - 0.8) .' 2" style="'. $this->config['STYLE'][0]['WIDGET_RACE'][0]['HIGHLITE_SELF_STYLE'][0] .'" substyle="'. $this->config['STYLE'][0]['WIDGET_RACE'][0]['HIGHLITE_SELF_SUBSTYLE'][0] .'"/>';
				}
				if ($item['rank'] != false) {
					// $item['rank'] is set 'false' in Team to skip the highlite here in $this->buildLiveRankingsWidget()
					$xml .= '<quad posn="-2 -'. ($this->config['LineHeight'] * $line + $offset - 0.3) .' 0.004" sizen="2 2" style="'. $this->config['STYLE'][0]['WIDGET_RACE'][0]['HIGHLITE_SELF_STYLE'][0] .'" substyle="'. $this->config['STYLE'][0]['WIDGET_RACE'][0]['HIGHLITE_SELF_SUBSTYLE'][0] .'"/>';
					$xml .= '<quad posn="-1.8 -'. ($this->config['LineHeight'] * $line + $offset - 0.1) .' 0.005" sizen="1.6 1.6" style="Icons64x64_1" substyle="ShowRight2"/>';
					$xml .= '<quad posn="'. $widgetwidth .' -'. ($this->config['LineHeight'] * $line + $offset - 0.3) .' 0.004" sizen="2 2" style="'. $this->config['STYLE'][0]['WIDGET_RACE'][0]['HIGHLITE_SELF_STYLE'][0] .'" substyle="'. $this->config['STYLE'][0]['WIDGET_RACE'][0]['HIGHLITE_SELF_SUBSTYLE'][0] .'"/>';
					$xml .= '<quad posn="'. ($widgetwidth + 0.2) .' -'. ($this->config['LineHeight'] * $line + $offset - 0.1) .' 0.005" sizen="1.6 1.6" style="Icons64x64_1" substyle="ShowLeft2"/>';
				}
			}
		}
	...
	public function getConnectedPlayerRecord ($login, $line, $topcount, $behind_rank = false, $widgetwidth) {
		...
		if ( isset($aseco->server->players->player_list[$login]) ) {
			...
			if (($line+1) > $topcount) {
				...
				$xml .= '<quad posn="0.4 -'. ($this->config['LineHeight'] * $line + 2.7) .' 0.004" sizen="'. ($widgetwidth - 0.8) .' 2" style="'. $this->config['STYLE'][0]['WIDGET_RACE'][0]['HIGHLITE_OTHER_STYLE'][0] .'" substyle="'. $this->config['STYLE'][0]['WIDGET_RACE'][0]['HIGHLITE_OTHER_SUBSTYLE'][0] .'"/>';
			}

			// Add a marker for Player with an record here
			$xml .= '<quad posn="-2 -'. ($this->config['LineHeight'] * $line + 2.7) .' 0.004" sizen="2 2" style="'. $this->config['STYLE'][0]['WIDGET_RACE'][0]['HIGHLITE_OTHER_STYLE'][0] .'" substyle="'. $this->config['STYLE'][0]['WIDGET_RACE'][0]['HIGHLITE_OTHER_SUBSTYLE'][0] .'"/>';
			$xml .= '<quad posn="'. $widgetwidth .' -'. ($this->config['LineHeight'] * $line + 2.7) .' 0.004" sizen="2 2" style="'. $this->config['STYLE'][0]['WIDGET_RACE'][0]['HIGHLITE_OTHER_STYLE'][0] .'" substyle="'. $this->config['STYLE'][0]['WIDGET_RACE'][0]['HIGHLITE_OTHER_SUBSTYLE'][0] .'"/>';
			if ($behind_rank == true) {
			...
			$xml .= '<quad posn="-1.7 -'. ($this->config['LineHeight'] * $line + 3) .' 0.005" sizen="1.4 1.4" style="'. $marker['style'] .'" substyle="'. $marker['substyle'] .'"/>';
			$xml .= '<quad posn="'. ($widgetwidth + 0.3) .' -'. ($this->config['LineHeight'] * $line + 3) .' 0.005" sizen="1.4 1.4" style="'. $marker['style'] .'" substyle="'. $marker['substyle'] .'"/>';
	...
	*/
	...
	public function buildDedimaniaRecordsWindow () {
		global $aseco;
	...
	*/
	...
	public function buildLocalRecordsWindow ($page) {
		global $aseco;
	...
	*/
	...
	public function buildLiveRankingsWindow ($page) {
		global $aseco;
	...
	*/
	...
	public function buildScorelistWindowEntry ($page, $widget, $fieldnames) {
		global $aseco;
	...
	*/
	...
	public function buildLastCurrentNextMapWindow () {
		...
		$xml .= '<quad posn="1.5 -23 0.04" sizen="2 2" image="file://Skins/Avatars/Flags/'. $this->cache['Map']['Last']['author_nation'] .'.dds"/>';
		...
		$xml .= '<quad posn="1.5 -23 0.04" sizen="2 2" image="file://Skins/Avatars/Flags/'. $this->cache['Map']['Current']['author_nation'] .'.dds"/>';
		...
		$xml .= '<quad posn="1.5 -23 0.04" sizen="2 2" image="file://Skins/Avatars/Flags/'. $this->cache['Map']['Next']['author_nation'] .'.dds"/>';
	...
	*/
	...
	public function buildMaplistWindow ($page, $player) {
		...
		if ( is_array($player->data['RecordsEyepiece']['Maplist']['Filter']) ) {
			if ( isset($player->data['RecordsEyepiece']['Maplist']['Filter']['environment']) ){
				// Filter for environment
				foreach ($this->cache['MapList'] as $map) {
					if (strtoupper($map['environment']) == $player->data['RecordsEyepiece']['Maplist']['Filter']['environment']) {
						$maplist[] = $map;
					}
				}
				unset($map);
				$listoptions = '(Filter: Only env. '. ucfirst(strtolower($player->data['RecordsEyepiece']['Maplist']['Filter']['environment'])) .')';
			}
				...
				if ($player->data['RecordsEyepiece']['Maplist']['Filter']['cmd'] == 'NORANK') {
					...
					foreach ($this->cache['MapList'] as $map) {
						if ( isset($player->data['RecordsEyepiece']['Maplist']['Records'][$map['uid']]) ) {
							if ( ($aseco->server->gameinfo->mode == Gameinfo::STUNTS) && ($player->data['RecordsEyepiece']['Maplist']['Records'][$map['uid']]['score'] <= $map['authortime_filter']) ) {
								$maplist[] = $map;
							}
					...
					foreach ($this->cache['MapList'] as $map) {
						if ( isset($player->data['RecordsEyepiece']['Maplist']['Records'][$map['uid']]) ) {
							if ( ($aseco->server->gameinfo->mode == Gameinfo::STUNTS) && ($player->data['RecordsEyepiece']['Maplist']['Records'][$map['uid']]['score'] <= $map['goldtime_filter']) ) {
								$maplist[] = $map;
							}
					...
					foreach ($this->cache['MapList'] as $map) {
						if ( isset($player->data['RecordsEyepiece']['Maplist']['Records'][$map['uid']]) ) {
							if ( ($aseco->server->gameinfo->mode == Gameinfo::STUNTS) && ($player->data['RecordsEyepiece']['Maplist']['Records'][$map['uid']]['score'] <= $map['silvertime_filter']) ) {
								$maplist[] = $map;
							}
					...
					foreach ($this->cache['MapList'] as $map) {
						if ( isset($player->data['RecordsEyepiece']['Maplist']['Records'][$map['uid']]) ) {
							if ( ($aseco->server->gameinfo->mode == Gameinfo::STUNTS) && ($player->data['RecordsEyepiece']['Maplist']['Records'][$map['uid']]['score'] <= $map['bronzetime_filter']) ) {
								$maplist[] = $map;
							}
				...
				foreach ($this->cache['MapList'] as $map) {
					if (
						...
						||
						(stripos($map['name_stripped'], $player->data['RecordsEyepiece']['Maplist']['Filter']['key']) !== false)
						||
						(stripos($map['filename'], $player->data['RecordsEyepiece']['Maplist']['Filter']['key']) !== false)
					)
					{
						$maplist[] = $map;
					}
					$listoptions = '(Search: &apos;'. $this->handleSpecialChars($player->data['RecordsEyepiece']['Maplist']['Filter']['key']) .'&apos;)';
				}
				unset($map);
			}
		}
...
//			$buttons .= '<quad posn="6.6 -1 0.12" sizen="3 3" action="showMaplistWindow" style="Icons64x64_1" substyle="ToolUp"/>';
		...
		if (count($maplist) > 0) {
			...
			for ($i = ($page * 20); $i < (($page * 20) + 20); $i ++) {
				...
				$player->data['RecordsEyepiece']['Maplist']['CurrentDisplay'][] = $map;
	...
	*/
	...
	public function buildManiaExchangeMapInfoWindow () {
		...
		$xml .= '<quad posn="2.9 -33.5 0.04" sizen="2.2 2.2" image="file://Skins/Avatars/Flags/'. $this->cache['Map']['Current']['author_nation'] .'.dds"/>';
			...
			if ($state == 'race') {
				...
				$xml = str_replace(
					...
					array(
						...
						$this->cache['Map']['Current']['author_nation']
					),
...
EOL;
	...
	*/
	...
	public function buildToplistWindowEntry ($data, $logins) {
		global $aseco;
		...
		if ( count($this->scores[$data['list']]) > 0) {
			...
			foreach ($this->scores[$data['list']] as $item) {
				if ($data['list'] == 'TopNations') {
					...
					// Mark current connected Players
					if ( (isset($item['login'])) && (in_array($item['login'], $logins)) ) {
						$xml .= '<quad posn="0.4 '. (((1.75 * $line - 0.2) > 0) ? -(1.75 * $line - 0.2) : 0.2) .' 0.01" sizen="16.95 1.83" style="'. $this->config['STYLE'][0]['WIDGET_RACE'][0]['HIGHLITE_OTHER_STYLE'][0] .'" substyle="'. $this->config['STYLE'][0]['WIDGET_RACE'][0]['HIGHLITE_OTHER_SUBSTYLE'][0] .'"/>';
					}
	...
	*/
	...
	public function buildToplistWindow ($page = 0) {
		global $aseco;
		...
		foreach (range(($page*4),($page*4+3)) as $id) {
			if ( isset($toplists[$id]) ) {
				...
				$xml .= $this->buildToplistWindowEntry($toplists[$id], $players);
				$xml .= '</frame>';
				$pos ++;
			}
		}
	...
	public function buildHelpWindow ($page) {
		...
		if ($page == 0) {
			...
			if ($this->config['FEATURES'][0]['MARK_ONLINE_PLAYER_RECORDS'][0] == true) {
				$xml .= '<quad posn="0.1 -38.9 0.01" sizen="2 2" style="'. $this->config['STYLE'][0]['WIDGET_RACE'][0]['HIGHLITE_SELF_STYLE'][0] .'" substyle="'. $this->config['STYLE'][0]['WIDGET_RACE'][0]['HIGHLITE_SELF_SUBSTYLE'][0] .'"/>';
				...
				$xml .= '<quad posn="0.1 -41.7 0.01" sizen="2 2" style="'. $this->config['STYLE'][0]['WIDGET_RACE'][0]['HIGHLITE_SELF_STYLE'][0] .'" substyle="'. $this->config['STYLE'][0]['WIDGET_RACE'][0]['HIGHLITE_SELF_SUBSTYLE'][0] .'"/>';
			...
			$xml .= '<quad posn="0.1 -44.5 0.01" sizen="2 2" style="'. $this->config['STYLE'][0]['WIDGET_RACE'][0]['HIGHLITE_SELF_STYLE'][0] .'" substyle="'. $this->config['STYLE'][0]['WIDGET_RACE'][0]['HIGHLITE_SELF_SUBSTYLE'][0] .'"/>';
...
EOL;
	...
	//	$header .= '<quad posn="0 0 0.01" sizen="4.5 4.5" action="closeMainWindow" style="Icons64x64_1" substyle="ArrowUp"/>';
		...
		$header .= '<label posn="6.8 -55.8 0.04" sizen="16 2" halign="center" valign="center" textsize="1" scale="0.7" action="showHelpWindow" focusareacolor1="0000" focusareacolor2="FFF5" textcolor="000F" text="RECORDS-EYEPIECE/'. $this->getVersion() .'"/>';
...
EOL;
	...
	*/
	...
	public function getCurrentRanking ($limit, $start) {
		...
		return $aseco->server->rankings->ranking_list;
...
//		if ( !empty($mapinfos) ) {
			...
			foreach ($aseco->server->maps->map_list as $mapob) {
				...
				$map['author_nation']	= $this->getMapAuthorNation($mapob->author_zone, $mapob->author);
				...
				if ($aseco->server->gameinfo->mode == Gameinfo::STUNTS) {
					$map['authortime']		= $this->formatNumber($mapob->authorscore, 0);
					...
					$map['authortime_filter']	= $mapob->authorscore;
					...
					// All other GameModes
					$map['authortime']		= $aseco->formatTime($mapob->authortime);
					...
					// Unformated for Maplist-Filter
					$map['authortime_filter']	= $mapob->authortime;
					$map['goldtime_filter']		= $mapob->goldtime;
					$map['silvertime_filter']	= $mapob->silvertime;
					$map['bronzetime_filter']	= $mapob->bronzetime;
				}
...
//	*/
//
//	public function readMapInfoGBX ($filename) {
//		global $aseco;
//
//		// Parse the GBX Mapfile
//		if ($this->config['FEATURES'][0]['MAPLIST'][0]['MAPIMAGES'][0]['ENABLED'][0] == true) {
...
//		}
//		catch (Exception $e) {
//			trigger_error('[RecordsEyepiece] Could not read Map ['. $aseco->server->mapdir . $aseco->stripBOM($filename) .'] at $this->readMapInfoGBX(): '. $e->getMessage(), E_USER_WARNING);
//
//			// Ignore if Map could not be parsed
//			return $this->getEmptyMapInfo();
//		}
...
//
//		$map['name']		= $this->handleSpecialChars($aseco->stripBOM($gbx->name));
//		$map['name_stripped']	= $aseco->stripColors($aseco->stripBOM($gbx->name), true);	// $aseco->stripColors() from basic.inc.php
//		$map['author']		= $aseco->stripBOM(trim($gbx->author));
//		$map['author_nation']	= $this->getMapAuthorNation($gbx->authorZone, $map['author']);
//		$map['uid']		= $gbx->uid;
...
//		if ($this->config['FEATURES'][0]['MAPLIST'][0]['MAPIMAGES'][0]['ENABLED'][0] == true) {
//
//			$path = $this->config['FEATURES'][0]['MAPLIST'][0]['MAPIMAGES'][0]['STORING_PATH'][0];
//			$filename = $gbx->uid .'.jpg';
//			if ($fh = fopen($path.$filename, "wb")) {
//				fwrite($fh, $gbx->thumbnail);
//				fclose($fh);
//
//				$map['imageurl'] = $this->config['FEATURES'][0]['MAPLIST'][0]['MAPIMAGES'][0]['ACCESS_URL'][0] . $filename;
//			}
//		}
//
//
...
//
//		if ($aseco->server->gameinfo->mode == Gameinfo::STUNTS) {
//			$map['authortime']		= $this->formatNumber($gbx->authorScore, 0);
//			$map['goldtime']		= $this->formatNumber($gbx->goldTime, 0);
//			$map['silvertime']		= $this->formatNumber($gbx->silverTime, 0);
//			$map['bronzetime']		= $this->formatNumber($gbx->bronzeTime, 0);
//
//			// Unformated for Maplist-Filter
...
//		else {
//			// All other GameModes
//			$map['authortime']		= $aseco->formatTime($gbx->authorTime);
//			$map['goldtime']		= $aseco->formatTime($gbx->goldTime);
//			$map['silvertime']		= $aseco->formatTime($gbx->silverTime);
//			$map['bronzetime']		= $aseco->formatTime($gbx->bronzeTime);
//
//			// Unformated for Maplist-Filter
...
//	}
	...
	*/
	...
	public function getMapAuthorNation ($authorzone, $author) {
		global $aseco;

		// First look into our Cache
		if ( ( isset($this->cache['MapAuthorNation'][$author]) ) && ($this->cache['MapAuthorNation'][$author] != 'other') ) {
			return $this->cache['MapAuthorNation'][$author];
		}


		// Remove BOM-header from AuthorZone
		$authorzone = $aseco->stripBOM($authorzone);
		...
		if ( (strpos($authorzone, '|') === false) && (isset($author)) && ($author == 'Nadeo') ) {
			...
			if ( ($nation !== false) && (!preg_match('/^Other Countries.*/i', $nation)) ) {
				$zone = $aseco->country->countryToIoc($nation);
				if ($zone == 'SCG') {
